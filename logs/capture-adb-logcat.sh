...
if [[ -z "${LOG_FILTER:-}" ]]; then
  BASE_LOG_FILTER='\[(JS|RUST)\] \[(PHOTO|ASSET|SYNC|DB|UI|PRINTER|AUTH|POS|SETTINGS):|asset\.localhost|convertFileSrc|preview_state_changed|pick_image_requested|pick_image_completed|pick_image_failed|pick_gallery_to_product_photo_input|preview_image_failed_to_load|get_cached_asset_path|get_pending_asset_preview|get_pending_asset_preview_path|asset_cache_ready|asset_attachment_ready|asset protocol not configured to allow the path|temp_photo_cleanup|upload_pending_assets|upload_pending_assets:start|upload_pending_assets:pending|upload_pending_assets:done|upload_asset:finalize_failed|process_pending_asset_jobs|asset_processing_job:failed|asset_processing_jobs|hydrate_missing_assets|pending_asset|manual_sync_requested|manual_sync_succeeded|manual_sync_failed|polling_starting|polling_started|polling_start_failed|headers_set|headers_set_failed|status_changed|status_get_state_failed|pull_batch|deleted_ids|soft_delete_row|push_batch|sync_push|sync_now|marked_rejected_outbox_synced|server_newer|payload_too_large|local_hard_limit|classifySyncError|sync_batch_requests|sync_cursors|last_server_event_id|sync_outbox|sync_updated_at|sync_proto|recordLocalChange|create_category|create_product|insert_category|insert_product|snapshot_export_requested|snapshot_export_finished|snapshot_export_failed|snapshot_export_done|ImagePipelinePlugin|imagepipeline|compressImage|generatePreview|\[baresync\]'
  LOG_FILTER="$BASE_LOG_FILTER" # Keep sync plus the full image preview/upload pipeline, the UI/DB path that produces outbox rows, and the plugin's own Rust setup/sync logs.
fi
