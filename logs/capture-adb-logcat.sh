...
if [[ -z "${LOG_FILTER:-}" ]]; then
  BASE_LOG_FILTER='\[(JS|RUST)\] \[(PHOTO|ASSET|SYNC|DB|UI|PRINTER|AUTH|POS|SETTINGS):|image_pipeline://|IMAGE-PIPELINE|asset\.localhost|convertFileSrc|preview_state_changed|pick_image_requested|pick_image_completed|pick_image_failed|pick_image_command_invoked|pick_image_command_returned|listeners_starting|listeners_started|preview_path_received|preview_url_resolved|job_completed_buffered|job_failed_buffered|job_completed_applied|job_failed_applied|preview_image_failed_to_load|preview_image_loaded|pick_gallery_to_product_photo_input|get_cached_asset_path|get_pending_asset_preview|get_pending_asset_preview_path|asset_cache_ready|asset_attachment_ready|asset protocol not configured to allow the path|temp_photo_cleanup|upload_pending_assets|upload_pending_assets:start|upload_pending_assets:pending|upload_pending_assets:done|upload_asset:finalize_failed|process_pending_asset_jobs|asset_processing_job:failed|asset_processing_jobs|hydrate_missing_assets|pending_asset|manual_sync_requested|manual_sync_succeeded|manual_sync_failed|polling|PICKER_STAGE_REQUEST|PICKER_STAGE_DONE|stage_picker_source_requested|stage_picker_source_done'
