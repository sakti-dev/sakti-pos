...
if [[ -z "${LOG_FILTER:-}" ]]; then
  BASE_LOG_FILTER='\[(JS|RUST|KOTLIN)\] \[(PHOTO|ASSET|SYNC|DB|UI|PRINTER|AUTH|POS|SETTINGS):|\[DIALOG\]|image_pipeline://|IMAGE-PIPELINE|ImagePipelinePlugin|native_pick|stage_content_uri|NATIVE_PICK_MEDIASTORE|asset\.localhost|convertFileSrc|preview_state_changed|pick_image_requested|pick_image_completed|pick_image_failed|pick_image_command_invoked|pick_image_command_returned|compress_asset_command_invoked|compress_asset_command_returned|delete_asset_command_invoked|delete_asset_command_returned|old_asset_deleted|old_asset_delete_failed|asset_compressed_event_received|preview_path_received|preview_url_resolved|job_completed_applied|job_failed_applied|preview_image_failed_to_load|preview_image_loaded|get_cached_asset_path|get_pending_asset_preview|get_pending_asset_preview_path|asset_cache_ready|asset_attachment_ready|asset protocol not'
