...
import { getStaffMember, updateStaffMember } from "~/db/staff";
...
export default function UserForm() {
  ...
  const canSubmitCreate = createMemo(() => {
    ...
    if (createFormInstance.isSubmitting) {
      return false;
    }
    return (
      !!input.name?.trim() &&
  ...
  const canSubmitEdit = createMemo(() => {
    ...
    if (editFormInstance.isSubmitting) {
      return false;
    }
    return !!input.name?.trim() && !!input.role?.trim();
  });

  const checkBusinessRules = (): string | null => {
          ...
          >
            ...
            >
              ...
              <button
                ...
                onClick={() => navigate(`/users/${params.id}/reset-pin`)}
                type="button"
              >
                Ubah PIN
              </button>
