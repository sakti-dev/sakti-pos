...
import { ApiError, register as cloudRegister } from "~/lib/cloud-auth";
import {
  CloudRegisterSchema,
  type CloudRegisterValues,
...
export default function CloudRegister() {
  ...
  const handleSubmit = async (values: CloudRegisterValues) => {
    setError("");
    try {
      await cloudRegister(values.email, values.password, values.name);
      ...
      >
        ...
        >
          ...
          <Button class="w-full" disabled={authForm.isSubmitting} type="submit">
            {authForm.isSubmitting ? "Memproses..." : "Daftar"}
          </Button>
