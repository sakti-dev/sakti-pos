...
export const verifyPin = (userId: number, pin: string) =>
	provider.verify(userId, pin);
export const hashPin = (pin: string) => provider.hashPin(pin);
export const changePin = (userId: number, newPin: string) =>
	provider.changePin(userId, newPin);
...
export async function seedDefaultOwner() {
	...
	try {
		...
		throw new Error(`Failed to seed default owner: ${String(err)}`);
	}
}
