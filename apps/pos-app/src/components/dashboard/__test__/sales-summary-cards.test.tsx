...
const UPPER_TRIANGLE_PERCENT = /\u25B2 \+25%/;
const LOWER_TRIANGLE_PERCENT = /\u25BC -20%/;
const NEW_LABEL = /\u25B2 Baru/;
...
describe("SalesSummaryCards", () => {
  ...
  test("shows positive delta with up arrow", () => {
    ...
    expect(getByText(UPPER_TRIANGLE_PERCENT)).toBeInTheDocument();
  });
  ...
  test("shows negative delta with down arrow", () => {
    ...
    expect(getByText(LOWER_TRIANGLE_PERCENT)).toBeInTheDocument();
  });
  ...
  test("shows Baru when previous is zero but current is not", () => {
    ...
    expect(getByText(NEW_LABEL)).toBeInTheDocument();
  });
