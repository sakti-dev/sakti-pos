...
describe("RevenueChart", () => {
	test("renders hourly title", () => {
		const { getByText } = render(() => (
			<RevenueChart data={HOURLY_DATA} type="hourly" loading={false} />
		));
	...
	test("renders daily title", () => {
		const { getByText } = render(() => (
			<RevenueChart data={DAILY_DATA} type="daily" loading={false} />
		));
	...
	test("renders weekly title", () => {
		const { getByText } = render(() => (
			<RevenueChart data={WEEKLY_DATA} type="weekly" loading={false} />
		));
	...
	test("renders monthly title", () => {
		const { getByText } = render(() => (
			<RevenueChart data={MONTHLY_DATA} type="monthly" loading={false} />
		));
	...
	test("shows empty state when all zeros", () => {
		...
		const { getByText } = render(() => (
			<RevenueChart data={emptyHourly} type="hourly" loading={false} />
		));
	...
	test("renders nothing when data is undefined and not loading", () => {
		const { container } = render(() => (
			<RevenueChart data={undefined} type="hourly" loading={false} />
		));
		expect(
