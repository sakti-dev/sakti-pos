...
Mobile and offline networks still retry requests, and server-visible row-state
changes can grow large enough that a single pull response is wasteful.
...
Make pull cursor-paged by row-state watermark with a bounded page size so the
client can advance through large changed-row sets without fetching everything at
once. ADR 0009 supersedes the older event-id cursor model.
...
Treat full resync as a baseline pull from an empty row-state cursor, not as a
continuation from the locally stored cursor.
...
Pulls can advance in smaller committed chunks, which reduces memory pressure and
keeps the sync flow resilient when the changed-row window is large.
