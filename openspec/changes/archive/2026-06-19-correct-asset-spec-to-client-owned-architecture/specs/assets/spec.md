...
#### Scenario: New asset created locally
...
#### Scenario: Plugin reports compression complete
...
#### Scenario: Compressed asset uploaded to object storage
...
#### Scenario: Asset row synced from API
...
#### Scenario: Client requests presigned upload URL
...
#### Scenario: API does not consult or write the assets table
...
#### Scenario: Client invokes pick_image
...
#### Scenario: Render picked image before compression completes
...
#### Scenario: Client invokes compress_asset
...
#### Scenario: Resolve cached asset to renderable URL
...
#### Scenario: Delete cached asset file
...
#### Scenario: Valid presign request with merchant access
...
#### Scenario: User lacks merchant access
...
#### Scenario: Run upload queue for a merchant
...
#### Scenario: Presign request or PUT fails
...
#### Scenario: Startup recovery for stuck pending rows
...
#### Scenario: Compress asset completes
...
#### Scenario: Client receives job_completed
...
The removed requirement described a `pending_asset_processing_jobs` table. The system SHALL not maintain such a table; job tracking is handled by the `assets.status` column.
...
The removed requirement described a `local_asset_cache` table. The system SHALL not maintain such a table; the plugin's filesystem cache is opaque to the application.
...
The removed requirement described a `POST /api/assets/complete-upload` endpoint. The system SHALL not expose such an endpoint; status transitions are client-owned.
