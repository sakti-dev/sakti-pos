...
The system SHALL guide new users through merchant creation, outlet creation, and owner PIN setup.

- Steps: `merchant` → `outlet` → `setup-pin`.
- If `merchantId` is in the query, the system skips to `outlet` or `setup-pin` as appropriate.
...
- If an owner staff already exists for the merchant, the system skips PIN setup and navigates to `/login`.
...
**WHEN** a user completes merchant and outlet creation with no existing owner staff
**THEN** the system prompts for PIN setup (two-entry confirmation).

**WHEN** a user enters matching PINs
**THEN** the system creates an owner staff row, claims it via cloud-to-staff mapping, syncs, and logs the user in.

**WHEN** an existing owner staff is found after outlet creation
**THEN** the system navigates to `/login` without PIN setup.
