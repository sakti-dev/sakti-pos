...
Cross-platform Google OAuth authentication for the Sakti POS desktop (Tauri) and Android apps. The flow opens the system browser for Google sign-in, uses a short-lived exchange code bridged via a custom deep-link scheme (`sakti-pos-dev://`), and reuses the existing post-auth flow (merchant/outlet selection). This keeps the long-lived session token out of any browser-accessible location and works uniformly on Android, macOS, Windows, and Linux.

## Requirements

### R1: System Browser Authentication
...
### R5: Deep-Link URL Routing in Rust
...
### R6: Global Deep-Link Listener in Frontend
